#!/usr/bin/env node
/* bacteria/data/bacteria.json 驗證器（零相依，供本機與 GitHub Actions 使用）
   執行： node scripts/validate_bacteria.js  失敗時 exit 1。 */
const fs = require('fs');
const path = require('path');
const FILE = path.join(__dirname, '..', 'bacteria', 'data', 'bacteria.json');
...
let raw, data;
try { raw = fs.readFileSync(FILE, 'utf8'); } catch (e) { console.error('✗ 無法讀取 bacteria/data/bacteria.json'); process.exit(1); }
